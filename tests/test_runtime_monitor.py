...
def test_model_failure_persists_only_stable_diagnostic_code(tmp_path):
    ...
    event = store.list_monitor_events(lane_id="lane-a")[-1]
    assert event["reason_code"] == "LLM_UNAVAILABLE" and not event["effective"]
...
def test_gap_resets_confirmation_and_no_action_is_not_markdown(tmp_path):
    ...
    assert not md.exists()  # Data observations are not effective trading signals.
