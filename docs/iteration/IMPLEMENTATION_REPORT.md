...
新增补充源政策和可运行的离线旁路审计入口。四类候选能力（题材梯队、主营F10、备用报价、观点导入）全部默认禁用、仅影子、无执行权；真实 provider、上游和许可证未核实的保持 `LIVE_UNVERIFIED`。候选能力注册复用 S02 ProviderRegistry，未授权 capability 无法通过 Governor；归一化事实复用现有 FactEnvelope，主营缺口可转换为 S04 CoverageObservation，没有另建生产事实库。
