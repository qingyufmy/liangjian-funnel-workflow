...
| OPS-01 | REPRODUCED_FIXED | 基线统一入口对 replay/shadow 只返回“未实现”；容易把缺证据与代码失败混淆 | S11 支持哈希证据包和明确 0/1/2/3/4 返回码；缺 manifest 与测试包实测返回 3 |
| OPS-02 | REPRODUCED_FIXED | RuntimeStore 有增量迁移但缺统一的副本迁移、二次幂等和回滚证据 | S11 本地测试 DB 演练通过，源哈希不变；注入中断保留完整 rollback 副本；生产一致性副本仍待授权 |
| OPS-03 | NEEDS_RUNTIME_EVIDENCE | 本机故障测试覆盖关键逻辑，但没有 VM 连续五日影子和真实故障恢复窗口 | S11 报告器禁止单日/测试数据验收；当前 test-only 单日报告返回 3 |
