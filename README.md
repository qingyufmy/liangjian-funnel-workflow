...
- **A1 宏观/产业链/基本面**：以产业链节点为研究单位，建立宽口径结构池，不读价格、不做技术面入场判断。单批 5 只只是模型传输边界，不是全局名额；每个输入必须恰好归入 `ACTIVE` / `MONITOR` / `REJECT` 之一。正式默认输入 120 只，可配到 300 只。
...
因此，A1 数量少不应通过降低证据标准来“凑数”。正确处理是保留足够宽的 G0 输入、取消批处理造成的全局截断，并把证据不足标的明确放入 `MONITOR` 或 `REJECT`，再由 A2/A3 完成市场与技术面收窄。
...
正式默认 G0 后取成交额最高且事实/因子就绪的 120 只，对齐 A1 结构研究池下限；初始域仍保存全市场计数和淘汰血缘。可用 `LIANGJIAN_RESEARCH_MAX_CANDIDATES` 调整到 1–300。上面的 `--max-candidates 20` 只用于首次能力/链路验收，不代表生产 A1 宽度。
完整 A1 默认每 5 只一批（`LIANGJIAN_A1_BATCH_SIZE`），模型阶段默认总时限 600 秒、最大输出 6,000 tokens。这些限制只影响模型投影；完整冻结快照与哈希不会被截断。
