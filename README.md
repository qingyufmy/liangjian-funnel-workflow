...
- **A1 宏观/产业链/基本面**：G0 只提供中性的可研究股票域；A1 AI 必须按“官方政策/宏观变化 → 结构性主题 → 产业链节点 → 公司主营与财务传导”选择 `ACTIVE`。它不读价格、不做技术面入场判断；T3 资讯只能发现线索，不能单独证明主题或公司受益。单批 8 只只是模型传输边界，不是全局名额；每个输入必须恰好归入 `ACTIVE` / `MONITOR` / `REJECT` 之一。正式默认输入 120 只，可配到 300 只。
- **A2 主题/情绪/市场角色**：只在 A1 `ACTIVE` 内判断市场正在交易什么，评估板块宽度、资金、梯队、周期与龙头/中军辨识度，将结构池收窄成聚焦池。
...
正式默认在 G0 后先按同花顺 881* 一级行业均衡覆盖、再按 884* 细分节点轮询，从每个已选节点内以流动性排序取 Top-N，冻结事实/因子就绪的 120 只。节点选择不使用当日成交热度，避免把 A2 的题材/情绪职责前移到 A1。初始域仍保存全市场计数和淘汰血缘。可用 `LIANGJIAN_RESEARCH_MAX_CANDIDATES` 调整到 1–300。上面的 `--max-candidates 20` 只用于首次能力/链路验收，不代表生产 A1 宽度。
完整 A1 默认按产业节点每 8 只一批（`LIANGJIAN_A1_BATCH_SIZE`），模型阶段默认总时限 600 秒、最大输出 6,000 tokens。这些限制只影响模型投影；完整冻结快照与哈希不会被截断。
