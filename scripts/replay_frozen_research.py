...
from liangjian_funnel.pipeline.research import (
    ...
    _with_a3_deterministic_context,
...
def main() -> int:
    ...
    if args.rotation_snapshot_overlay:
        ...
        expected_hash = _canonical_hash(snapshot_data)
        snapshot_id = f"{snapshot_id}:rotation-evidence-validation"
...
def _resume_stage(
    ...
    *,
    ...
    # Match the production A3 path exactly: the model and the post-model
    # threshold/provenance policy both require the immutable server-owned
    # strategy context generated by the deterministic gate.  Without this
    # overlay an isolated historical resume can falsely demote a technically
    # qualified row as A3_STRATEGY_CONTEXT_MISSING even though production
    # would retain its ALLOW_A4 route.
    stage_snapshot = _with_a3_deterministic_context(stage_snapshot, gate)
